...
var FriendCard_info = cc.Layer.extend({
    ...
    ctor: function(data,layer,ruleIndex,sourceName) {
        ...
        this._isAntiAddictionMode = this._data ? (this._data.info.isOpenAntiAddiction & 1) :false;
        this._clubType = 0//this._data ?  this._data.info.type : 0; //房卡俱乐部 普通俱乐部
        ...
        this.back.getChildByName("play_isArrearage").setVisible(false);
        //创建俱乐部时
    ...
    updateWanfa:function () {
        ...
        for (var i = 1; i <= this._ruleBtnNum; i++) {
            if (MjClient.RuleParam["rule" + i] && MjClient.RuleParam["rule" + i].ruleName) {
                ...
                if(ruleName.length > 6){
                    let ruleNameLength = parseInt(ruleName.length / 2);
                    ruleName = ruleName.slice(0,ruleNameLength) + '\n' + ruleName.slice(ruleNameLength)
                }
    ...
    initSelectLayer: function(node) {
        ...
        back_selectType.setVisible(false)
    ...
    initLayer: function(node, data, layer, ruleIndex, sourceName) {
        ...
        this._textNotice.setTouchEnabled(true);
        ...
        this._btn_setPlay.addTouchEventListener(function(sender, type) {
            if (type == 2) {
                ...
                var isAntiAddictionMode = 0;
                if(that._clubType == 0){
                    isAntiAddictionMode = 1;
                }
            postEvent("createRoom", {
                    ...
                    isAntiAddictionMode:isAntiAddictionMode
                });
            }
        });
